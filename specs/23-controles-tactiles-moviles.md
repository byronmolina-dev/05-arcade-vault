...
## Amendments (2026-07-24, gamepad neón solo para Tetris)

Se agregó una variante visual "gamepad" a los controles táctiles, exclusiva de Tetris, inspirada en `references/gamepad-assets/gamepad.html`, que reemplaza los dos clusters flotantes (`.touch-dpad`/`.touch-actions`) de ese juego por un único panel de control (D-pad en cruz con hub central + dos botones circulares de acción cyan/magenta).

- **`lib/games/touchControls.ts`**: nuevo campo opcional `variant?: "classic" | "gamepad"` en `TouchControlsConfig` (default `"classic"`, sin cambio para `asteroides`/`bloque-buster`/`serpentina`). `tetris` pasa a `variant: "gamepad"`. Los `TouchButtonConfig` (code/label/discrete) no cambian — la variante solo afecta el render.
- **`components/games/TouchControls.tsx`**: cuando `config.variant === "gamepad"`, renderiza `GamepadControls` en vez de los clusters clásicos. Reusa exactamente el mismo `pointerHandlers`/auto-repeat que la variante clásica. Mapeo: D-pad con los botones de `config.dpad` posicionados por `code` (◄/▼/► de Tetris; el slot "up" del D-pad queda vacío porque Tetris no tiene movimiento `ArrowUp`); las dos `actions` de Tetris (ROTAR, CAER) se muestran como botones circulares B (cyan, izquierda) y A (magenta, derecha) con iconos ⟳/⤓ en vez de texto.
- **`app/globals.css`**: nuevas clases `.touch-controls--gamepad .gp`, `.gp-body`, `.gp-dpad`, `.dp*`, `.dp-hub*`, `.gp-actions`, `.ab*` — paleta y proporciones tomadas del asset de referencia, reutilizando `--cyan`/`--magenta`/`@keyframes pulse-led` ya existentes en el tema (coinciden exactamente con los colores del asset, sin necesidad de nuevas variables).
- **Alcance**: solo Tetris. `asteroides`, `bloque-buster` y `serpentina` no se tocan (siguen con la variante `"classic"` de siempre).
- Verificado con Playwright en viewport móvil (390×844, `ontouchstart` forzado): el panel se renderiza reemplazando los controles anteriores, el botón de rotar cambia la forma de la pieza activa y el D-pad izquierdo mueve la pieza repetidamente contra la pared, sin pausar ni romper el loop del juego.

**Ajuste 2026-07-24 (feedback en dispositivo real):** la primera versión superponía el panel como overlay absoluto sobre `.crt-screen` (igual que la variante clásica), tapando parte del tablero y el panel NEXT/SCORE. Se cambió a panel en flujo normal del documento, debajo de `.crt-screen` (dentro de `.crt`, antes de `.crt-bottom`) — `components/GamePlayerClient.tsx` ahora decide dónde montar `<TouchControls>` según `TOUCH_CONTROLS_CONFIG[game.id]?.variant` (dentro de `.crt-screen` para la variante clásica, debajo para `"gamepad"`). También se redujo el ancho máximo del panel (`max-width: 300px`, centrado con `.touch-gamepad-dock`) y el tamaño de D-pad/botones A-B, para que no se estire de borde a borde en un teléfono angosto.
